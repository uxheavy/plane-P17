...
} from "@/components/work-maps/editor/scene";
...
describe("Work map scene boundary", () => {
  it("keeps materialized images when a sibling asset fetch fails", async () => {
    ...
    try {
      const { materializeFiles } = await import("@/components/work-maps/editor/assets");
  ...
  it("retries transient reads and same-epoch CAS but stops at an authority error", async () => {
    ...
    vi.doMock("@/components/work-maps/editor/merge-authoritative-scene", () => ({
    ...
    try {
      const [{ usePersistence }, { WorkMapService }] = await Promise.all([
        import("@/components/work-maps/editor/use-persistence"),
  ...
  it("keeps image serialization pending through capped transient upload retries", async () => {
    ...
    vi.doMock("@/components/work-maps/editor/assets", () => ({
      addWorkMapAssetMetadata: (files: unknown) => files,
      materializeFiles: vi.fn().mockResolvedValue({}),
      uploadFile,
    }));
    vi.doMock("@/components/work-maps/editor/scene", () => ({
    ...
    try {
      const { useScene } = await import("@/components/work-maps/editor/use-scene");
      ...
      vi.doUnmock("@/components/work-maps/editor/assets");
      vi.doUnmock("@/components/work-maps/editor/scene");
      vi.resetModules();
      vi.useRealTimers();
      vi.unstubAllGlobals();
    }
  ...
  it("applies scenes before image recovery and rejects stale materialization", async () => {
    ...
    vi.doMock("@/components/work-maps/editor/assets", () => ({
      materializeFiles: materializeFilesMock,
      uploadFile: vi.fn(),
    }));
    vi.useFakeTimers();
    vi.stubGlobal("window", { setTimeout, clearTimeout, requestAnimationFrame: vi.fn() });
