...
const lockManager = (): TLockManager | null => {
  ...
  const locks = (navigator as Navigator & { locks: Partial<TLockManager> }).locks;
  return typeof locks?.request === "function" ? (locks as TLockManager) : null;
};
...
export const withRecoveryWriterLock = async <T>(
  ...
  return locks.request(writerLockName({ accountId, workMapId, writerId }), { ifAvailable }, async (lock) => {
...
export const createRecoveryWriter = ({ accountId, workMapId }: TRecoveryScope): TRecoveryWriter => {
  ...
  let readyResolve: ((available: boolean) => void) | undefined;
  let ready = locks ? new Promise<boolean>((resolve) => (readyResolve = resolve)) : Promise.resolve(true);
  ...
  const activate = () => {
    ...
    registered = true;
    ready = new Promise<boolean>((resolve) => (readyResolve = resolve));
    const pendingRequest = lockRequest;
    if (pendingRequest) void pendingRequest.then(() => claimLock());
    else claimLock();
  };
  revocations.add(revoke);
  writersByAccount.set(accountId, revocations);
