...
describe("Work map recovery boundary", () => {
  ...
  it("marks a live writer so another tab does not auto-replay its recovery slot", () => {
    installStorage();
    vi.stubGlobal("crypto", { randomUUID: () => "writer-a" });
    const writer = createRecoveryWriter({ accountId: "user-id", workMapId: "map-id" });
    writer.retain(1, recoveryScene("active"));
    expect(readActiveRecoveryWriterIds("user-id", "map-id")).toEqual(new Set(["writer-a"]));
    writer.release();
    expect(readActiveRecoveryWriterIds("user-id", "map-id")).toEqual(new Set());
  });
