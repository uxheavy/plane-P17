...
export const usePersistence = (context: TContext, scene: TSceneOwners) => {
  ...
  const queue = useCallback(
    ...
    [persistenceFailed, retainJournal, retainRecovery, save, scene]
  );
  ...
  useEffect(() => {
    ...
    let activeWriterIds: Set<string>;
    try {
      activeWriterIds = readActiveRecoveryWriterIds(userId, workMapId);
    } catch {
      return;
    }
    for (const record of recoveryRecords) {
      if (
        recoveryStates[record.writerId]?.status !== "replayable" ||
        automaticRecoveryRef.current.has(record.writerId) ||
        activeWriterIds.has(record.writerId)
      )
        continue;
      automaticRecoveryRef.current.add(record.writerId);
      void recoveryRetryRef.current(record.writerId, true);
    }
  }, [recoveryRecords, recoveryStates, userId, workMapId]);

  useEffect(() => {
    if (!userId || typeof window === "undefined") return;
    const onRecoveryStorage = (event: StorageEvent) => {
      if (!event.key?.startsWith("work-map-recovery")) return;
      try {
        setRecoveryRecords(readRecovery(userId, workMapId));
      } catch {
        // The existing mounted recovery path reports storage errors when it next refreshes.
      }
    };
    window.addEventListener("storage", onRecoveryStorage);
    return () => window.removeEventListener("storage", onRecoveryStorage);
  }, [userId, workMapId]);
  ...
  const hasPendingDraft = useCallback(() => pendingRef.current !== null, []);
