...
class FileAssetEndpoint(BaseAPIView):
    ...
    def post(self, request, slug):
        ...
        if serializer.is_valid():
            if serializer.validated_data.get("entity_type") in {
                FileAsset.EntityTypeContext.WORK_MAP_SCENE,
                FileAsset.EntityTypeContext.MESSAGE_ATTACHMENT,
            }:
    ...
    def delete(self, request, workspace_id, asset_key):
        asset_key = str(workspace_id) + "/" + asset_key
        file_asset = FileAsset.objects.exclude(
            entity_type__in=[
                FileAsset.EntityTypeContext.WORK_MAP_SCENE,
                FileAsset.EntityTypeContext.MESSAGE_ATTACHMENT,
            ]
...
class UserAssetsEndpoint(BaseAPIView):
    ...
    def get(self, request, asset_key):
        ...
        ).filter(asset=asset_key, created_by=request.user)
        if files.exists():
    ...
    def delete(self, request, asset_key):
        ...
        ).get(asset=asset_key, created_by=request.user)
        file_asset.is_deleted = True
        file_asset.save(update_fields=["is_deleted"])
        return Response(status=status.HTTP_204_NO_CONTENT)
